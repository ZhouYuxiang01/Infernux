...
- `recipes/control_runtime.md` - drive Play Mode through generic
  `ControlSignal` input.
- `recipes/safe_world_edit.md` - make bounded Edit Mode changes and verify
  them with world diffs.
- `recipes/debug_runtime_errors.md` - inspect runtime, script, scene, and MCP
  failures.

The engine is not the agent. It exposes the world state, lifecycle, control,
events, bounded editing, and verification surfaces that external agents use to
operate the project.
