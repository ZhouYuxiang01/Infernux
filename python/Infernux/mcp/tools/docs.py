...
CONCEPTS: dict[str, dict[str, Any]] = {
    ...
    "AI Runtime Core": {
        ...
        "tools": ["agent_bootstrap", "runtime_explain_current_scene", "runtime_get_world_snapshot", "runtime_capture_game_render_target", "runtime_experiment_begin"],
    },
    "Visual Observation": {
        "summary": "Pixel-level capture of engine-rendered Game View output for external agents.",
        "notes": [
            "Use runtime_capture_game_render_target when camera framing, sprite orientation, UI layout, or rendering output matters.",
            "Use runtime_capture_game_view only as a visible-window fallback for editor presentation diagnostics.",
            "Use runtime_get_world_snapshot for structured state; visual capture is a companion check, not the source of truth.",
        ],
        "tools": ["runtime_capture_game_render_target", "runtime_capture_game_view", "runtime_get_world_snapshot", "runtime_read_errors"],
    },
    "World Model": {
...
def _agent_bootstrap_payload(project_path: str, *, agent_name: str = "", task_intent: str = "") -> dict[str, Any]:
    return {
        ...
        "startup_sequence": [
            ...
            {"tool": "runtime_capture_game_render_target", "why": "Capture engine-rendered pixels when rendering or layout matters."},
...
def _operating_loop() -> list[str]:
    return [
        "Observe: mcp_health, runtime_explain_current_scene, runtime_get_world_snapshot, runtime_capture_game_render_target when visual output matters, scene_query_objects.",
        ...
        "Verify: read object/world state, capture the Game Render Target when relevant, diff snapshots, runtime_assert, runtime_read_errors.",
    ...
    return [
        {
            ...
            "tools": ["mcp_health", "runtime_explain_current_scene", "runtime_get_world_snapshot", "runtime_capture_game_render_target", "scene_query_objects"],
        },
        {
