...
def register_runtime_tools(mcp) -> None:
    ...
    @mcp.tool(name="runtime_capture_game_view")
    def runtime_capture_game_view(output_path: str = "") -> dict:
        """Capture the currently visible Game View viewport as a PNG."""

        def _capture():
            from Infernux.ai_runtime.visual_observation import capture_game_view

            return capture_game_view(output_path=output_path)
